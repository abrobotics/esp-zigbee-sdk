...
static size_t rmt_encode_ir_nec(rmt_encoder_t *encoder, rmt_channel_handle_t channel, const void *primary_data, size_t data_size, rmt_encode_state_t *ret_state)
{
    ...
    const uint8_t *payload = (const uint8_t *)primary_data;
    ...
    // fall-through
    case 1: // send payload bytes (address/command length is determined by data_size)
        encoded_symbols += bytes_encoder->encode(bytes_encoder, channel, payload, data_size, &session_state);
        if (session_state & RMT_ENCODING_COMPLETE) {
